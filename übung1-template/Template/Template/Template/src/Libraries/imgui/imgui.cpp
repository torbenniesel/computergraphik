...
/*
 ...
 - A minimal render function skeleton may be:
    ...
    void void RenderFunction(ImDrawData* draw_data)
    {
 ...
 - 2017/12/24 (1.53) - renamed the emblematic ShowPluginWindow() function to ShowDemoWindow(). Kept redirection function (will obsolete).
    ...
    if (g.NavWindow)
        if (ImGuiWindow* focused_root_window = g.NavWindow->RootWindow)
            if (focused_root_window->WasActive && focused_root_window != window->RootWindow)
            {
                ...
                // NB: The order of those two Plugins is important because Modal windows are also Popups.
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags)
{
    ...
    // Plugin for bounding box overlap, as updated as ItemAdd()
    if (!(window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect))
        return false;
    IM_ASSERT((flags & (ImGuiHoveredFlags_RootWindow | ImGuiHoveredFlags_ChildWindows)) == 0);   // Flags not supported by this function

    // Plugin if we are hovering the right window (our window could be behind another window)
    ...
    // Plugin if another item is active (e.g. being dragged)
    if (!(flags & ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
        if (g.ActiveId != 0 && g.ActiveId != window->DC.LastItemId && !g.ActiveIdAllowOverlap && g.ActiveId != window->MoveId)
            return false;

    // Plugin if interactions on this window are blocked by an active popup or modal 
    if (!IsWindowContentHoverable(window, flags))
        return false;

    // Plugin if the item is disabled
...
// Plugin if mouse cursor is hovering given rectangle
...
static void SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond)
{
    // Plugin condition (NB: bit 0 is always true) and clear flags for next time
